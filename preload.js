...
contextBridge.exposeInMainWorld('api', {
    ...
    send: (channel, data) => {
        let validChannels = ['save-servers', 'log-to-system', 'start-server', 'stop-server', 'open-folder', 'kill-server', 'send-command', 'show-server-gui', 'get-player-count', 'open-docs'];
    ...
    invoke: (channel, data) => {
        let validChannels = ['get-servers', 'get-settings', 'check-admin', 'open-dialog', 'select-folder', 'read-config-file', 'write-config-file'];
        if (validChannels.includes(channel)) {
